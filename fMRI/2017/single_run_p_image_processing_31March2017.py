...
#matplotlib.rcParams.update({'ytick.major.width': 5})
...
for fig in plt_dic:
	...
	use_labels = False
	if use_labels:
		plt.ylabel(plt_dic[fig][1], labelpad=20)
		plt.xlabel(plt_dic[fig][0], labelpad=0)
		title(plt_dic[fig][2])
...
for s in subjects.keys():
	for p in sorted(painting_dic[s].keys()):
		...
		plot(2,1,'Dk',markersize=m)
		plot(2,0.5,'sk',markersize=m)
		plot(2,0,'ok',markersize=m)
		plot(2,0.5,'sk',markersize=m)
		plot(2,0,'ok',markersize=m)
		print(d2f('\t',s,dp(ttest(vv,fv),n),dp(ttest(vf,ff),n),dp(ttest(vv,rv),n),dp(ttest(rf,ff),n),dp(ttest(vf,rf),n),dp(ttest(rv,fv),n),p))
